...
**App**
- grannydb now **opens and works offline**, which is the difference between crocheting on a plane
  with the app and crocheting on a plane with a notebook. Two mechanisms, deliberately separate: a
  service worker precaches the app shell so the page loads, and the last dataset read on the device
  is saved so there is something to show. Neither one touches what the other is responsible for.
- The load falls back to that saved snapshot when every read path throws. Before, no signal meant no
  data at all — all three read paths need the network, and the authenticated one needs it twice
  (the blob cache is keyed by a commit sha you can only learn from a request), so `reload()` ended
  in `phase: 'error'` and every screen, being built from `snapshot.schemas`, collapsed to one error
  line. The edits already queued on the phone were safe on disk with nothing to render them. The
  cache is keyed by `{owner}/{repo}/{branch}/{dataDir}`, so a scratch branch cannot serve you
  `main`'s rows, and what comes back out is shape-checked rather than trusted, since some older
  build wrote it. A stale base is safe to draw because it is only ever a base: screens replay the
  queue over it, and a sync re-reads the repo fresh before committing, so nothing derived from it
  reaches GitHub. See [ADR 0026](adr/0026-last-known-good-snapshot-for-offline-reads.md).
- Staleness is visible rather than implied: a line above the nav ("Offline — showing the copy
  saved on this device, 3 hours ago"), a `cache` entry in Settings → Read from, and a new "Data as
  of" row built from `fetchedAt` — a field every snapshot has carried and nothing read until now.
- The app is **installable** — a web manifest and icons, so it lives on the home screen rather
  than in a tab you have to remember not to close. The icons are generated by `npm run build-icons`
  (zero dependencies: a granny square is concentric bands of colour, and Node already has zlib)
  with the output committed, rather than regenerated on every deploy.
- The service worker is hand-written, about seventy lines, instead of `vite-plugin-pwa`. It caches
  the document and the hashed JS/CSS and nothing else — cross-origin requests are passed through
  untouched, so no authenticated GitHub response can land in a cache on an origin shared with every
  other page under the same github.io user, and `data/bundle.json` is left out so there are not two
  competing answers to "how old is this". It does not take over on its own either: a new build
  surfaces as a "A newer version is ready" bar, because swapping the running app out mid-edit is
  worse than being an hour behind. See
  [ADR 0027](adr/0027-hand-written-service-worker-and-install.md).
- When the device comes back online the app **re-reads**, and only re-reads — guarded to the case
  where what is on screen came from the cache. Syncing stays a button you press; the reasoning in
  the sync engine doc is unchanged, and the asymmetry is the argument: a read behind your back is
  invisible and reversible, a commit to your repo is neither.
- A failed connection is now `NetworkError` rather than a bare `TypeError: Failed to fetch`
  propagating untyped, so the sync screen says you appear to be offline instead of showing a
  browser's internal wording. An `AbortError` passes through untouched — a cancelled request is
  deliberate, not a flat tyre.
- Tests cover both halves: the load falling back to the saved copy when every read path throws, and
  the worker itself run in a fake worker global — including that a cross-origin request is never
  intercepted, which is prose everywhere else and is the one property worth having enforced.
- Settings → "Reload from GitHub" now does what its label always claimed. It said it cleared the
  cached copy of the repo on this device; it called `reload()`, which cleared nothing. It now
  forgets the saved snapshot first.

**Docs**
- [ADR 0026](adr/0026-last-known-good-snapshot-for-offline-reads.md) on the snapshot cache (and why
  not service-worker-cached API responses, a precached bundle, or the existing blob cache), and
  [ADR 0027](adr/0027-hand-written-service-worker-and-install.md) on the hand-written worker and
  what it is allowed to cache.
- [Storage layer](03-storage-layer.md) now contrasts the two caches directly — one keyed by sha
  and never stale but unreachable without the network, one keyed by data location and always
  available — and the read-paths table gains the fallback. [Operations](06-operations.md) covers
  the new build outputs, the production-only worker, and the iOS caveat that an installed web app
  has its own
  storage: the token has to be pasted again inside it, and its queue is a different queue.
- [Overview](01-overview.md) drops "offline install (PWA)" from what v1 does not do.
