...
A second plugin emits `dist/sw.js` — the service worker from [`src/sw/sw.js`](../src/sw/sw.js) with
the hashed asset filenames and a version derived from them substituted in, since the build is the
only place those names are known. Everything in `public/` (the web manifest and the icons) is copied
to the root of `dist/` as it is. Together those make the app installable and able to open with no
network; see [ADR 0027](adr/0027-hand-written-service-worker-and-install.md).

The icons are **not** regenerated by the pipeline. `npm run build-icons` writes them from
[`scripts/build-icons.ts`](../scripts/build-icons.ts) and the output is committed, because they
change roughly never and a new binary in every deploy is noise.
...
**The service worker is production-only.** In development the app unregisters any worker it finds
instead of installing one, because a precached shell served over the dev server's own files is a
confusing afternoon. To exercise offline behaviour, use `npm run preview` — that is a production
build — and DevTools → Network → Offline.

When a change seems not to have arrived, there are now two caches to think about and they fail
differently: a stale *shell* means the page itself is a deploy behind, and the app offers a
"A newer version is ready" bar when it knows about one. A stale *dataset* is the saved snapshot, and
Settings → Reload from GitHub forgets it and reads again.
...
4. Optional, and the point of the offline work: add it to the home screen (iOS: Share → Add to Home
   Screen; Android: the install prompt). Open it once with signal so it has a copy of the data to
   fall back to.

**An installed app on iOS has its own storage, separate from Safari's.** The token, the pending
queue and the saved snapshot in the installed app are not the tab's. So paste the token again inside
it, and sync before switching between the two — pending edits made in one are invisible to the
other. Nothing is lost either way; they are just two devices as far as the sync engine is
concerned.
...
- **A CSP in [`index.html`](../index.html)** restricting `connect-src` to `api.github.com` and
  `raw.githubusercontent.com`, `script-src` (and `worker-src`, `manifest-src`) to `'self'`, with
  `base-uri 'none'` and `frame-ancestors 'none'`. If script does run, exfiltration is constrained.
- **The service worker caches the app shell only.** Cross-origin requests are passed through without
  being intercepted at all, so no authenticated GitHub response is ever written to a cache on this
  shared origin.
